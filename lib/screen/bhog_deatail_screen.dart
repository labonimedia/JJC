...
class BhogDetailsScreen extends StatefulWidget {
  @override
  _BhogPaymentState createState() => _BhogPaymentState();
}

class _BhogPaymentState extends State<BhogDetailsScreen> {
  ...
  void initState() {
    ...
    print("fdsfd" + currentDateOnly.toString());
                            ...
                            children: [
                              ...
                              Row(
                                ...
                                children: [
                                  Padding(
                                    padding:
                                        EdgeInsets.only(left: 45, right: 20),
                                    child: Text(
                                      "Name: " + userName,
                                      style: TextStyle(
                                        color: BlackColor,
                                    ...
                                    : alloption == "paidForAll"
                                        ? GestButton(
                                            ...
                                            onclick: () {
                                              ...
                                              if (status == "0") {
                                                showToastMessage(
                                                    "Please contact admin your payment status is pending!");
                                              } else {
                                                bookingConfirm();
                                              }
                                            },
                                                  ...
                                                  if (status == "0" && partialCount <= (int.parse(fmembercount) - int.parse(colected))) {
                                                    if (partialCount > 0 && isAllSelected == false) {
                                                      ...
                                                      if ((int.parse(fmembercount) - int.parse(colected)) ==
                                                          0) {
                                                        showToastMessage(
                                                            "You have check-in for family!. You need to pay for guest");
                                                      } else {
                                                        showToastMessage(
  ...
  Future<void> bookingConfirm() async {
    final Uri uri = Uri.parse(Config.baseurl + Config.bhogBooking);
  ...
  Future<BookingResponse> getbookingInfo() async {
    final Uri uri = Uri.parse(Config.baseurl + Config.bhogBookingInfo);
    final response = await http.post(
      uri,
      body: jsonEncode({
        "uid": userId,
