...
class EventDetailsScreenKclub extends StatefulWidget {
  const EventDetailsScreenKclub({super.key});

  @override
  State<EventDetailsScreenKclub> createState() => _EventDetailsScreenState();
}

class _EventDetailsScreenState extends State<EventDetailsScreenKclub> {
          ...
          return bookStatus == "0"
            ...
            children: [
              ...
              GestButton(
                ...
                buttontext: "Audience Booking".tr,
                style: TextStyle(
                  color: WhiteColor,
                ...
                child: Builder(
                  builder: (context) {
                    return GestureDetector(
                      onTap: () {
                        List<EventType> eventTypeData = eventDetailsController.eventInfo?.eventypelist ?? [];

                        if(responseData?.familyList.isEmpty ?? true) {
                          if (eventTypeData.isNotEmpty) {
                            _showMultiSelectPopup(context,
                                eventTypeData); // Now context is correct!
                          } else {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                  content: Text("No event types available")),
                            );
                          }
                        }else{
                          ScaffoldMessenger.of(context).showSnackBar(
                          ...
                          Get.to(() => TicketQrCodePerformer(
                              ticketData:
                              '0;${eventId};${responseData?.familyList.first.days};$eventId;${responseData?.familyList.first.event_type};0;${getData.read("UserLogin")["name"]};${getData.read("UserLogin")["id"].toString()};${responseData?.familyList.first.status};${formatDate(eventDetailsController.eventInfo?.eventData!.eventSdate ?? '')};${eventDetailsController.eventInfo?.eventData?.alloption}',eventId: eventId,eventype:"kclubPerformer" ));
                        }
                      },
                      child: Padding(
              ...
              /*  ElevatedButton(
                ...
                onPressed: () {}, // Prevent direct button press
                child: Builder(

                  builder: (BuildContext context) { // Ensures correct BuildContext
                    return GestureDetector(
                      ...
                      child: Padding(
                        padding: const EdgeInsets.symmetric(vertical: 10),
                        child: Text("Book Performer", style: TextStyle(color: Colors.white)),
                      ),
                    );
                  },
                ),
