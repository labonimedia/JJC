...
class EventDetailsControllerKclub extends GetxController implements GetxService {
  ...
  getEventData({String? eventId}) async {
    try {
      ...
      Uri uri = Uri.parse(Config.baseurlKclub + Config.eventDetails);
      ...
      print("dsdsd123"+response.body);
      if (response.statusCode == 200) {
        var result = jsonDecode(response.body);
        menberList = [];
        for (var element in result["EventData"]["member_list"]) {
          menberList.add("${Config.imageUrlkclub}" + element);
        }
  ...
  getEventTicket({String? eventId}) async {
    try {
      ...
      Uri uri = Uri.parse(Config.baseurlKclub + Config.ticketApi);
      var response = await http.post(
        uri,
        body: jsonEncode(map),
      );
  ...
  getCatWiseEvent({String? catId, title, img}) async {
    try {
      ...
      print(map.toString());
      Uri uri = Uri.parse(Config.baseurlKclub + Config.catWiseEvent);
      var response = await http.post(
        uri,
        body: jsonEncode(map),
      );
  ...
  getFavAndUnFav({String? eventID}) async {
    try {
      ...
      Uri uri = Uri.parse(Config.baseurlKclub + Config.favORUnFav);
      var response = await http.post(
        uri,
        body: jsonEncode(map),
      );
